...
import type { RiskCaseItem, RiskEventItem, RiskRuleItem, UserBehaviorControlItem } from '@/api/modules/admin'

type RiskTab = 'events' | 'cases' | 'rules' | 'behavior'
...
<template>
  <div class="admin-panel space-y-6">
    ...
    <div v-if="!canViewRiskEvents && !canHandleRiskCase && !canManageRiskRule && !canViewBehaviorControl" class="py-16 text-center text-slate-400">
      当前账号暂无风控中心权限
    </div>
    ...
    <div v-else-if="riskTab === 'events' && canViewRiskEvents" class="space-y-4">
      <div class="flex flex-wrap gap-3 items-center p-4 bg-gradient-to-r from-slate-50 to-warm-50/60 rounded-2xl border border-warm-100">
        ...
        <el-input v-model="riskLevelModel" placeholder="按风险等级筛选（LOW/MEDIUM/HIGH/CRITICAL）" class="w-72" clearable />
        <input
          v-model="riskStartTimeModel"
          type="datetime-local"
          class="h-10 px-3 text-sm border border-warm-200 rounded-xl bg-white text-slate-700 outline-none focus:ring-2 focus:ring-warm-100 focus:border-warm-300"
          title="开始时间"
        />
        <input
          v-model="riskEndTimeModel"
          type="datetime-local"
          class="h-10 px-3 text-sm border border-warm-200 rounded-xl bg-white text-slate-700 outline-none focus:ring-2 focus:ring-warm-100 focus:border-warm-300"
          title="结束时间"
        />
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-warm-100">
            <tr v-for="event in riskEvents" :key="event.eventId">
              ...
              <td class="py-3 px-4">
                <span class="px-2 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-600">{{ event.riskLevel || '-' }}</span>
              </td>
          ...
          <tbody class="divide-y divide-warm-100">
            <tr v-for="item in riskCases" :key="item.caseId">
              ...
              <td class="py-3 px-4 text-right">
                <button
                  @click="emit('risk-case-process', item.caseId)"
                  class="px-4 py-2 bg-warm-500 text-white rounded-lg text-sm font-medium hover:bg-warm-600"
                >
                  处理
                </button>
              </td>
    ...
    <div v-else-if="riskTab === 'rules' && canManageRiskRule" class="space-y-4">
      <div class="flex gap-3 items-center p-4 bg-gradient-to-r from-slate-50 to-warm-50/60 rounded-2xl border border-warm-100">
        ...
        <button @click="emit('risk-rule-create')" class="px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-semibold hover:bg-emerald-600">
          新增规则
        </button>
      </div>
            ...
            <tr v-for="rule in riskRules" :key="rule.ruleId">
              ...
              <td class="py-3 px-4">
                <span
                  ...
                  :class="rule.status === 1 ? 'bg-emerald-50 text-emerald-600' : 'bg-warm-100 text-warm-700'"
                >
                  {{ rule.status === 1 ? '启用' : '禁用' }}
                </span>
              </td>
              <td class="py-3 px-4 text-right">
                <button
                  @click="emit('risk-rule-toggle', rule)"
                  class="px-4 py-2 rounded-lg text-sm font-medium"
                  :class="rule.status === 1 ? 'bg-white border border-red-200 text-red-500 hover:bg-red-50' : 'bg-emerald-500 text-white hover:bg-emerald-600'"
                >
                  {{ rule.status === 1 ? '禁用' : '启用' }}
                </button>
              </td>
    ...
    <div v-else-if="riskTab === 'behavior' && canViewBehaviorControl" class="space-y-4">
      <div class="flex gap-3 items-center p-4 bg-gradient-to-r from-slate-50 to-warm-50/60 rounded-2xl border border-warm-100">
        ...
        <button @click="emit('behavior-user-query')" class="px-5 py-2.5 bg-gradient-to-r from-warm-500 to-warm-400 text-white rounded-xl text-sm font-semibold hover:shadow-lg hover:shadow-warm-200 transition-all">
          查询
        </button>
        <button v-if="canManageBehaviorControl" @click="emit('behavior-control-add')" class="px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-semibold hover:bg-emerald-600">
          新增管控
        </button>
      </div>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-warm-100">
            <tr v-for="control in behaviorControls" :key="control.id">
              ...
              <td class="py-3 px-4 text-right">
                <button
                  v-if="canManageBehaviorControl"
                  @click="emit('behavior-control-disable', control.id)"
                  class="px-4 py-2 bg-white border border-red-200 text-red-500 rounded-lg text-sm font-medium hover:bg-red-50"
                >
                  关闭
                </button>
              </td>
    ...
    <div v-if="riskTab !== 'behavior' && total > 0" class="mt-2 flex justify-end">
      <el-pagination
